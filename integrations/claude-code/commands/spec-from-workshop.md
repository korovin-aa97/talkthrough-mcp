---
description: "Turn a recorded workshop or design walkthrough into a structured spec with quoted decisions and open questions."
---
...
walkthrough, processed by talkthrough as job `$ARGUMENTS`.

## Method

1. Read the whole discussion: get_transcript(job_id="$ARGUMENTS", format="text");
...
2. Identify decision points and requirement statements. For each, keep the
   ...
   get_transcript(job_id="$ARGUMENTS", start_ms=..., end_ms=...) when in doubt.
3. Whenever the discussion references something visual (whiteboard, mockup,
   screen), pull it: get_frames(job_id="$ARGUMENTS", at_ms=<that moment>) — or
   get_moment(job_id="$ARGUMENTS", start_ms=..., end_ms=...) for slice + frames +
   OCR in one call.
4. Use search(job_id="$ARGUMENTS", query="<term>") to collect every mention of a
