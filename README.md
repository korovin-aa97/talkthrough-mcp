...
- **[▶ Watch the demo with sound (1:18)](https://youtu.be/nHfGfEiVdE8)** — a
  real end-to-end session: a narrated recording goes in and a ready-to-file
  `bug-report.md` comes out. Processing is shown at approximately 5× speed.
...
**Fastest Claude Code path:** [install the full plugin](#claude-code), then run
`/talkthrough:bug /absolute/path/to/recording.mp4`. If you try it, share a
[successful recipe](https://github.com/korovin-aa97/talkthrough-mcp/issues/new?template=recipe.md)
or report a
[problem](https://github.com/korovin-aa97/talkthrough-mcp/issues/new?template=bug_report.md) —
you never need to attach a private recording.

## Quickstart

Once [uv](https://docs.astral.sh/uv/) is installed, talkthrough runs via
`uvx`: ffmpeg falls back to a bundled build, OCR is pip-only, and whisper
models download themselves on first use. Install uv with `brew install uv` or
