...
# can ask "who said what" without reinstalling (v0.2.0 decision), and since
# 0.4.0 the [url] extra so process_url works on YouTube links out of the box
# (yt-dlp plus a PyPI-distributed Deno runtime — no manual installs). The
# PACKAGE default stays lean — `uvx talkthrough-mcp` remains the documented
# minimal install (direct https:// media URLs work there too) — and both
# diarization and networking are still opt-in per call.
_PYPI_SPEC = "talkthrough-mcp[diarization,url]"
_GIT_SPEC = (
...
ENV_DOC = (
    ...
    "TALKTHROUGH_MAX_DOWNLOAD_BYTES (cap for `process_url` downloads, default 2 GiB). "
    "Speaker diarization is included but off per call — agents pass "
    "`diarize=true` (plus `num_speakers` when known). URL ingestion is the "
    "only tool that uses the network: `process_url(url)` downloads one public "
    "video/audio URL once (YouTube needs the `[url]` extra carried by this "
    "config); the minimal server without the diarization engine and without "
    f"YouTube support is `{MINIMAL_UVX_CMDLINE}`."
)

VERIFY_DOC = (
    "Verify: the client should list 9 tools (process_media, process_url, get_transcript, "
    "get_frames, get_moment, search, label_speakers, extract_frame, list_jobs). A `list_jobs` "
    "call returning an empty list is a healthy first run."
)
...
def build_registry_manifest() -> dict[str, str]:
    manifest = {
        # 2025-12-11 superseded 2025-09-29 (the Registry warned on the 0.3.2
        # publish). Same package/transport shape; the newer schema only adds
        # optional title/websiteUrl/icons/remotes. tests/unit/test_registry_manifest.py
        # validates the generated file against a vendored copy of the schema.
        "$schema": REGISTRY_SCHEMA,
        "name": "io.github.korovin-aa97/talkthrough-mcp",
        "title": "Talkthrough",
        # Registry hard limit: <= 100 chars (enforced server-side, checked 2026-07-11).
        "description": (
            "Local MCP server: recordings or public video URLs into transcript, "
            "frames, OCR, wall-clock evidence."
        ),
        ...
        "packages": [
            {
                ...
                "environmentVariables": [
                    ...
                    {
                        "name": "TALKTHROUGH_MAX_DOWNLOAD_BYTES",
                        "description": (
                            "Hard cap for one process_url download. Default: 2147483648 "
                            "(2 GiB)."
                        ),
                        "isRequired": False,
                        "isSecret": False,
